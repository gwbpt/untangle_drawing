...
from math import cos, sin, pi # a supprimer !!!!!!!!!!!!!
...
import GraphLogic1 as LG

class NodeTk(LG.Node):
    def __init__(self, canvas, id, xy, xy0, r, name=None):
        LG.Node.__init__(self, id, xy0, name)
        self.canvas = canvas
        self.r = r
        self.solved_x, self.solved_y = xy
        x, y = xy0
        self.x, self.y = x, y
        self.itemId = self.canvas.create_oval(x-r, y-r, x+r, y+r, fill="blue")
        self.txtId = self.canvas.create_text(x, y, text=name, fill="yellow")
    ...
    def getPos(self):
        ...
        '''
        x0, y0, x1, y1 = self.canvas.coords(self.itemId)
    ...
    def setPos(self, x, y):
        ...
        self.canvas.coords(self.itemId, x-self.r, y-self.r, x+self.r, y+self.r)
...
class LinkTk(LG.Link):
    def __init__(self, canvas, id, node0, node1, name=None, e=3):
        LG.Link.__init__(self, id, node0, node1, name=name)
        self.canvas = canvas
...
class GraphTk(LG.Graph):
    def __init__(self, canvas, id=0, name='', nodes=None, links=None, e=3):
        LG.Graph.__init__(self, id=id, name=name, nodes=nodes, links=links) # id=0, name='', nodes=None, links=None
...
class GuiGame(TK.Frame):
    def __init__(self, parent):
        ...
        for i, xy in enumerate(xys):
            ...
            print(i, xy_pix)
            node = NodeTk(self.canvas, i, xy_pix, xy0pix, rNode)
        ...
        for i, (n0, n1) in enumerate(lnks):
            ...
            node0 = self.nodes[n0]
            node1 = self.nodes[n1]
            lnk = LinkTk(self.canvas, i, node0, node1)
